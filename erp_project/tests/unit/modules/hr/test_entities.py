...
class TestEmployee:
    ...
    def test_update_salary(self):
        ...
        employee = Employee(
            ...
            first_name="John",
            surname="Doe",
            salary=Salary.usd_only(1000),
        )

        new_salary = Salary.create(usd_amount=1500, zig_amount=5000)
        old_salary = employee.update_salary(new_salary)

        assert old_salary.usd_amount == Decimal("1000")
        assert employee.salary.usd_amount == Decimal("1500")
        assert employee.salary.zig_amount == Decimal("5000")
    ...
    def test_years_of_service(self):
        ...
        # Should be at least 6 years (as of 2026)
        assert employee.years_of_service >= 6

    def test_contract_expired(self):
        """Test contract expired check."""
        employee = Employee(
            id=1,
            employee_id=EmployeeId("EMP0001"),
            first_name="John",
            surname="Doe",
            contract_to=date(2020, 1, 1),
        )

        assert employee.contract_expired

    def test_contract_not_expired(self):
        """Test contract not expired."""
        employee = Employee(
            id=1,
            employee_id=EmployeeId("EMP0001"),
            first_name="John",
            surname="Doe",
            contract_to=date(2030, 1, 1),
        )

        assert not employee.contract_expired
    ...
    def test_str_representation(self):
        ...
        assert "EMP0001" in str(employee)
