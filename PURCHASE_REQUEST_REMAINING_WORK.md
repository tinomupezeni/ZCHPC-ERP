...
- **Backend:** the Purchase Request–specific unit, policy, and use-case test suites pass in full, including all Director-authorization tests (25/25 in a targeted run covering `test_purchase_request_policy.py`, `test_purchase_request_use_cases.py`, and related entity/value-object tests). A broader, pre-existing legacy suite for unrelated `PurchaseOrder`/`BudgetCenter`/`Vendor` entities has failures that were confirmed to also exist identically on `main` — not introduced by this work.
