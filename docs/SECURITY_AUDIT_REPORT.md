...
*   **Issue:** A default temporary password (`erp@1234`) was hardcoded in source code and assigned to newly created users. This meant every new account started with the same, publicly-known password.
...
*   Replaced realistic-looking placeholder credentials (`erp@1234`) with obviously fake values (`CHANGE_ME_TO_A_STRONG_PASSWORD`) so developers and security scanners can clearly distinguish templates from real credentials.
